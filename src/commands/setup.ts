...
export default class SetupCommand extends Command {
  ...
  async execute(interaction: CommandInteraction) {
    ...
    if (!member.permissions.has('ADMINISTRATOR')) {
      interaction.reply('Error: insufficient permissions. Only Administrators may use this command.');
      return;
    }
    ...
    for (const batch of batchArray) {
      await batch
        .commit()
        .then((result) => {
          interaction.reply(`User batch ${batchCount} updated in Firebase successfully.`);
          console.log(result);
  
          // Output when all batches are processed.
          if (batchCount === batchIndex + 1) {
            interaction.reply('All batches processed. To enable karma tracking, please use the `setmeme` command.');
          }
          batchCount++;
        })
        .catch((err) => {
          interaction.reply(`Batch ${batchCount} has encountered an error. See console for more details.`);
          console.log(err);
          batchCount++;
        });
    }
