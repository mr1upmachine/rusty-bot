...
function getInsult(users: Users): Promise<string> {
  const gqlRequest = new GraphRequest(
    new GraphVariables(users),
    'getInsult',
    'query getInsult($users: Users!) { GetInsult(people: $users) { message }}',
  );
  return axios
    .post(`https://insult.appspot.com/graphql`, gqlRequest)
    .then(res => {
