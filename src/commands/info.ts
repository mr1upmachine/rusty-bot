...
exports.run = async (client: Client, msg: Message, args: string[], firestore: Firestore) => {
    ...
    .then((doc) => {
        if (!doc.exists) {
            ...
            if (doc.data()!.about) { about = doc.data()!.about; }
            if (doc.data()!.infoColor) { color = doc.data()!.infoColor; }
            if (doc.data()!.posts || doc.data()!.posts === 0) { postCount = doc.data()!.posts; }
            if (doc.data()!.karma || doc.data()!.karma === 0) { karma = doc.data()!.karma; }
...
function embedBuilder(msg: Message, pfp: string, userNick: string, about: string, postCount: number, karma: number, userColor: string) {
